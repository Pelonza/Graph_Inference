...
for graph in graphdic.keys():
    ...
    ax.axis([0,50,0,100])
    ax.plot(mon_percent, node_percent, 'g^',label="RPS")
    ax.plot(alg1_mon_percent, alg1_node_percent, 'bs', label='p=0')
    plt.legend(loc='best', fontsize=10)
    plt.grid(True)
    #plt.show()
    plt.savefig(graphdic[graph]+'_alg1fixed_RPS_nodes_over'+str(numRuns)+'Runs.pdf')
    ...
    ax.axis([0,50,0,100])
    ax.plot(mon_percent,edge_percent, 'g^',label="RPS")
    ax.plot(alg1_mon_percent,alg1_edge_percent, 'bs', label='p=0')
    plt.legend(loc='best', fontsize=10)
    plt.grid(True)
    #plt.show()
    plt.savefig(graphdic[graph]+'_alg1fixed_RPS_edges_over'+str(numRuns)+'Runs.pdf')
