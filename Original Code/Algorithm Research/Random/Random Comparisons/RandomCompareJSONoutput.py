...
#RPD
    outfile=open(graphdic[graph]+'_RPD_mon_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RPD_mon_percent,outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'_RPD_node_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RPD_node_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'_RPD_edge_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RPD_edge_percent, outfile)
    outfile.close()
#RPS
    outfile=open(graphdic[graph]+'_RPS_mon_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RPS_mon_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'_RPS_node_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RPS_node_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'_RPS_edge_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RPS_edge_percent, outfile)
    outfile.close()
#RW
    if graphdic[graph]!='General Relativity':
        outfile=open(graphdic[graph]+'_RW_mon_percent_ over'+str(numRuns)+' Runs', 'w')
        json.dump(RW_mon_percent, outfile)
        outfile.close()
        outfile=open(graphdic[graph]+'_RW_node_percent_ over'+str(numRuns)+' Runs', 'w')
        json.dump(RW_node_percent, outfile)
        outfile.close()
        outfile=open(graphdic[graph]+'_RW_edge_percent_ over'+str(numRuns)+' Runs', 'w')
        json.dump(RW_edge_percent, outfile)
        outfile.close()
#RWS
    outfile=open(graphdic[graph]+'_RWS_mon_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RWS_mon_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'_RWS_node_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RWS_node_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'_RWS_edge_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(RWS_edge_percent, outfile)
    outfile.close()
