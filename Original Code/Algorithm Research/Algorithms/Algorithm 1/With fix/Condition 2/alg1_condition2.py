...
import high_neighbor_degree_condition2L
import numpy as np
import json
graphdic={}
numRuns = 50
...
for graph in graphdic.keys():
    ...
    for i in range(numRuns):
        ...
        mo, no, ed = [],[],[]
        HNDc = high_neighbor_degree_condition2L.HND(graph,i)
        tally=0
    ...
    outfile=open(graphdic[graph]+'alg1c2_mon_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(alg1_mon_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'alg1c2_node_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(alg1_node_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'alg1c2_edge_percent_ over'+str(numRuns)+' Runs', 'w')
