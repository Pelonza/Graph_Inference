...
import math
import high_neighbor_degree_condition1_alg2
import sys
import numpy as np
import json
graphdic={}
...
for graph in graphdic.keys():
    ...
    for i in range(numRuns):
        ...
        mo, no, ed = [],[],[]
        HNDc = high_neighbor_degree_condition1_alg2.HND(graph,i)
        tally=0
    ...
    outfile=open(graphdic[graph]+'alg2c1_mon_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(alg2_mon_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'alg2c1_node_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(alg2_node_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'alg2c1_edge_percent_ over'+str(numRuns)+' Runs', 'w')
