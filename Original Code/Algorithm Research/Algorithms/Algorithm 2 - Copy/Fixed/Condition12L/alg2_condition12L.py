...
import math
import high_neighbor_degree_condition12L_alg2
import sys
import numpy as np
import json
graphdic={}
...
for graph in graphdic.keys():
    ...
    for i in range(numRuns):
        ...
        mo, no, ed = [],[],[]
        HNDc = high_neighbor_degree_condition12L_alg2.HND(graph,i)
        tally=0
    ...
    outfile=open(graphdic[graph]+'alg2c12L_mon_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(alg2_mon_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'alg2c12L_node_percent_ over'+str(numRuns)+' Runs', 'w')
    json.dump(alg2_node_percent, outfile)
    outfile.close()
    outfile=open(graphdic[graph]+'alg2c12L_edge_percent_ over'+str(numRuns)+' Runs', 'w')
