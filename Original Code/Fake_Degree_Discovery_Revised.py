...
class alg():
    ...
    def place_next_monitor(self, node, prob):
        #choice_list=[testnode for testnode in node_options if self.seen[testnode] != 'inf']

        seen_list=self.seen.keys()
        seen_list.remove('Total')
        seen_list=[testnode for testnode in seen_list if self.seen[testnode] != 'inf']
        #fake_max_degree_list=maxes(self.monitor_free, key=lambda mynode: self.fake_degree[mynode])   
