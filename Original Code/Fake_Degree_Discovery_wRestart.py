...
class alg():
    ...
    def place_next_monitor(self, node, prob):
        if self.no_new>=2:
            ...
            fake_max_degree_list=maxes(seen_list, key=lambda mynode: self.fake_degree[mynode])
            best_node_list=maxes(fake_max_degree_list, key=lambda mynode: self.seen[mynode])
