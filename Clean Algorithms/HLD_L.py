...
class Alg():
    '''
    Algorithm: Highest-Local Degree with Restart/Walk Limit-Log (HLD_rL)
    ...
    Current Algorithm Behavior:
        ...
        Place Next:
            Check Walk-Length. If greater than log(nodes_in_graph), restart
            Else
    ...
    def place_next_monitor(self, node, param):
        if self.walklen>self.maxwalklen:
            #Check walk length, and restart if max-len bexceeded.
