...
class Alg():
    '''
    Algorithm: Hill-Climbing with Probablistic Restart and 1/2 Neighbors Seen(HC_pF)
    ...
    Place_Next:
        Examine neighbors
        If 1/2 neighbors have monitor, teleport/pick new start
...
'''
    ...
    def add_neighbors(self, node):
        ...
        for neighbor in neighbors:
            ...
            if degree not in list(self.next_highest.keys()) and neighbor not in self.monitor_set:
    ...
    def _empty_check(self, key):
        if len(self.next_highest[key])==0:
            ...
            if key==self.highseen_key and list(self.next_highest.keys()):
...
#        else:
        ...
        neighbor_testset = set(neighbors) - self.monitor_set
            
        #Check how many neighbors we have to pick from.
        if len(neighbor_testset)< len(neighbors)/2:
            #If less than half, teleport
            next_monitor=self.pick_start()
        else:
            #Otherwise, do hill-climbing with probability of restarting
            for neighbor in neighbors:
            ...
            if len(neighbor_set)>0:
                ...
                if prob >= magic_number:     
                    ...
                    if not list(self.next_highest.keys()):
